...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    ...
    category: 'Health & maintenance',
    keywords: ['trash', 'undo delete', 'restore', 'deleted by mistake', 'recover catalog', 'recover addon', 'recently deleted', 'undo'],
    answer: 'Deleting a catalog or addon offers an Undo straight away, and it stays recoverable for 30 days under Tasks -> Maintenance -> Recently deleted.',
    steps: [
      'Right after deleting, press Undo on the confirmation message.',
      'Later on: Tasks -> Maintenance -> Recently deleted, then Restore.',
      'Delete forever removes it permanently - that one has nothing behind it, so it asks first.',
    ],
    details: [
      'Restoring brings the item back as it was. An addon comes back with its group assignments re-attached, skipping any group that has since been deleted rather than failing the whole restore.',
      'Anything not restored within 30 days is cleared automatically.',
      'This covers catalogs and addons. Deleting a user is still permanent: a user carries watch history, sessions and credentials, and a restore that quietly lost some of that would be worse than being told plainly that the delete is final.',
    ],
    tips: [
