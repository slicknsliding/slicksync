...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    ...
    category: 'Getting started',
    keywords: ['invite', 'invitation', 'add household member', 'invite link', 'invite nuvio user', 'nuvio invite'],
    ...
    details: [
      'Whoever opens the link chooses Stremio or Nuvio and signs in with that account. Which one they pick decides what kind of user the invite creates, so a Nuvio household can be built entirely through invitations.',
      'Signing in with Nuvio shows a short code to enter in the Nuvio app, the same device sign-in used when you connect an account yourself. The credential is exchanged on the server and never passes through the page.',
      'Their email comes from the account they signed in with, so there is nothing to type and nothing to get wrong. You still approve the request before any user is created.',
    ],
    href: '/invitations',
  ...
  },
  {
    ...
    category: 'Catalogs & Collections',
    keywords: ['nuvio collections manager', 'build collection', 'home screen folder', 'community covers', 'cover art nuvio', 'linked catalog', 'link catalog to folder', 'folder auto update', 'collections guard', 'collections disappeared', 'collections vanished', 'restore collections'],
  ...
  },
  {
    ...
    answer: 'SlickSync can serve a real Stremio addon of its own, per user: Continue Watching, the household Watchlist, and every Catalog appear as live rows inside Stremio and Nuvio on that user\'s devices. Quickest way in: Addons -> Browse - SlickTrax is the first row of the list. Press Configure, pick the user, press Add (or copy their manifest URL for a manual install anywhere). The per-user toggle also lives on the user\'s page under Watch-Tracking Integrations.',
    steps: [
      'Addons -> Browse -> SlickTrax is the first row: press Configure, pick the user, press Add.',
  ...
  },
  {
    ...
    details: [
      ...
      'Wipe is the permanent option: it deletes every watched episode of that show for that person, so watch time and stats stop counting it. The button asks a second time and names exactly how many episodes it is about to erase. The erased rows land in the Trash first, so even a wipe can be undone for 30 days from Tasks → Recent Changes.',
      'The "also remove from the account\'s library" checkbox above the graveyard makes a wipe reach the device too: the title is dropped from the provider account\'s own library, so it disappears from the device\'s home screen, not just from SlickSync. It applies to single and bulk wipes alike, and is skipped automatically for accounts whose library can\'t be edited from outside.',
    ],
    related: ['now-playing-continue-watching', 'rewatch-completion', 'trash-and-undo'],
  },
  {
  ...
  },
  {
    ...
    category: 'Watching & Discover',
    keywords: ['watchlist order', 'reorder watchlist', 'up next', 'watchlist ranking', 'sort watchlist'],
    answer: 'Drag the Watchlist into the order you actually want to watch things in. The top item is your "up next", and the SlickTrax Watchlist row on your devices follows the same order instead of the date things were added.',
    details: [
      'A watchlist nobody has reordered behaves exactly as it always did - newest additions first. Ranked items sort ahead of unranked ones, so you can rank the few you care about and leave the rest alone.',
      'The order is per household, not per device, so what you arrange here is what everyone sees in the addon row too.',
    ],
    related: ['watchlist-basics', 'slicktrax-addon'],
  },
  {
    id: 'trakt-list-import',
    title: 'Importing a public Trakt list',
    category: 'Catalogs & Collections',
    keywords: ['trakt list', 'import trakt', 'trakt client id', 'public list'],
    answer: 'Paste a trakt.tv list URL into Catalogs - Import. It needs a free Trakt Client ID in Settings - Integrations, and it reads PUBLIC lists only.',
    details: [
      'This is deliberately not a Trakt account connection. Trakt limits a free account to one connected app, so linking SlickSync would evict whatever Trakt app you already use. A Client ID reads public lists without touching that limit.',
      'Both URL shapes work: trakt.tv/users/<user>/lists/<slug> and trakt.tv/lists/<id>. The catalog is named after the list itself.',
      'Auto-refresh works on a Trakt source like any other import, so the catalog can keep following the list.',
      'Letterboxd and IMDb lists are not supported and cannot be: neither exposes a public list API, so the only way to read one is scraping their HTML, which breaks the moment they change a page.',
    ],
  ...
  },
  {
    ...
    category: 'Catalogs & Collections',
    keywords: ['anilist import', 'myanimelist import', 'mal list', 'anime list', 'import anime', 'planning list', 'mal client id'],
    answer: 'Catalogs - Import, then paste anilist.co/user/NAME/animelist or myanimelist.net/animelist/NAME. AniList needs no key; MyAnimeList needs a free Client ID. Both are matched to real titles with your TMDb key.',
    details: [
      ...
      'AniList needs nothing at all. MyAnimeList needs a free Client ID in Settings - Integrations, which reads public lists only and does not connect your MAL account - the same arrangement as Trakt. Jikan, the keyless MAL mirror, would have avoided that key, but MAL removed the endpoint it read lists from and it now answers list requests with a plain "MyAnimeList refuses to connect".',
  ...
  },
  {
    ...
    details: [
      ...
      'Stremio only, for now, which is why the Patch button does not appear on a Nuvio user. Nuvio stores just the addon address and fetches the manifest from Cinemeta itself, so a patched manifest would never reach the device. To get Cinemeta catalogs off a Nuvio home screen, hide those rows in the home-row editor instead.',
    ],
  ...
  },
  {
    ...
    category: 'Getting started',
    keywords: ['settings groups', 'settings tabs', 'where is themes', 'settings navigation'],
    answer: 'Settings is grouped: General, Themes, Sync, Notifications, Features, Integrations and Security. Themes lives here now rather than as its own page.',
    details: [
      'The groups appear as a left rail when the app is in the Top Navi layout, and along the top when it is in the Original layout - so there are never two vertical navigations side by side.',
      'The fastest way to a specific setting is the command palette: Ctrl+K (Cmd+K), type what you want - "digest", "2fa", "timezone" - and picking the result opens the right group and flashes the exact control.',
      'The old /themes address still works; it redirects into the Themes group.',
    ],
    related: ['command-palette', 'themes-and-layout'],
  },
  {
    id: 'smart-catalogs',
  ...
  },
  {
    ...
    category: 'Watching & Discover',
    keywords: ['anime', 'anilist', 'seasonal anime', 'airing', 'absolute episode', 'episode numbering', 'watch order', 'sub dub'],
  ...
  },
  {
    ...
    details: [
      'The episode calendar already covers a show while it is airing. This covers the gap afterwards - the wait between a finale and any news, which is exactly when people forget a show existed.',
      'Muting is per subject and keeps the follow: a muted show stops alerting but stays in the list, and un-muting will not replay news you already received.',
      'The first check after following is deliberately silent - it records where things stand rather than announcing the state the title was already in.',
      'Checks run twice a day. Renewals and new credits are not minute-scale news, and a gentler cadence keeps this from becoming another feed to ignore.',
      'Uses your own TMDb key from Settings. Without one, following still records your choices but no alerts can be raised.',
    ],
    related: ['notifications-setup', 'anime-support'],
  },
  {
    id: 'full-data-backups',
    title: 'Backing up your watch history (full-data backups)',
    category: 'Health & maintenance',
    keywords: ['full data backup', 'database backup', 'watch history backup', 'snapshot', 'restore database', 'sqlite backup', 'lose history', 'disk failure'],
    answer: 'Tasks → Maintenance → Full-data backups. Config backups protect your setup; this protects your watch history, which otherwise exists only in the live database. Turn it on and it snapshots the database on a schedule, verifies the snapshot is readable, and optionally sends an encrypted copy off-site.',
    steps: [
      'Tasks → Maintenance → Full-data backups → turn on Scheduled snapshots.',
      'Set how often (daily by default) and how many copies to keep on this box.',
      'To send them off-site as well, set an encryption passphrase in Off-site backups first, then turn on "Send snapshots off-site too".',
      'Press Snapshot now once to confirm it works - the result says the size and whether the snapshot verified as readable.',
    ],
    details: [
      'Why this is separate from config backups: a config backup carries users, groups, addons, catalogs and settings - everything to rebuild the setup - but no watch history. Continue Watching, For You, Taste Profiles, streaks and Year in Review are all built on that history, and without this lane it lives in exactly one place. A dead disk would restore the setup perfectly and lose every minute anyone ever watched.',
      'The snapshot is taken with SQLite\'s own consistent-copy mechanism while the app keeps running, then opened and checked before it is kept - so a corrupt copy is caught immediately rather than discovered during a restore.',
      'A snapshot is the WHOLE database, including the Vault\'s rows (whose secrets stay encrypted under this instance\'s own key). That is why an off-site copy refuses to upload without an encryption passphrase: config exports may travel in the clear if you choose, but the entire database may not.',
      'Restoring is deliberately not a button in the app. Replacing a database the running app is holding open is how a restore goes wrong. Download the snapshot, stop the container, and run scripts/restore-data-snapshot.js with the file (and --passphrase if it is encrypted). It moves the current database aside instead of deleting it, so the restore itself can be undone.',
      'Private instances only: in public mode the database is Postgres, backed up by whoever hosts it.',
    ],
    related: ['maintenance-and-updates', 'backup-restore'],
    href: '/tasks',
    linkLabel: 'Open Tasks',
  },
  {
    ...
    steps: [
      'Off-site backups: pick S3 or WebDAV, fill in the destination, then Test target to confirm it works before trusting it. An optional encryption passphrase makes every upload an encrypted .enc file - worthwhile because backups carry addon install URLs, which often embed API keys. Local backups stay plain (Time Machine and restores read them directly), and an .enc file imports through the normal Import with its passphrase. Lose the passphrase and the remote copies cannot be read.',
    ...
    details: [
      ...
      'These config backups carry no watch history - that is what the separate Full-data backups lane on this same page is for. Turn that on if the household record matters to you as much as the setup does.',
